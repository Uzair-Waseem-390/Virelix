...
// ✅ Moved outside TabTeam so it's not recreated on every render
const MemberCard = ({ type, project, editing, formData, setFormData, isSaving, error, success, onEdit, onSave, onCancel }) => {
...
const TabTeam = ({ project, onUpdate }) => {
    ...
    const handleSave = async (type) => {
        ...
        try {
            if (type === 'manager') {
                ...
                onUpdate({ ...project, manager_details: response.data.manager });
            } else {
                const response = await updateStaff(project.id, updates);
                setSuccess('Staff credentials updated successfully!');
                onUpdate({ ...project, staff_details: response.data.staff });
            }
            setEditing(null);
            setFormData({ email: '', password: '' });
        } catch (err) {
    ...
    const sharedProps = { project, editing, formData, setFormData, isSaving, error, success, onEdit: handleEdit, onSave: handleSave, onCancel: handleCancel };

    return (
        <div className="space-y-4">
            <MemberCard type="manager" {...sharedProps} />
            <MemberCard type="staff" {...sharedProps} />

            {(!project.manager_details || !project.staff_details) && (
