...
const MembersTable = ({ members, projects, onMemberUpdate }) => {
    ...
    const getProjectForMember = (memberId) => {
        const project = projects?.find(p =>
            p.manager_id === memberId || p.staff_id === memberId
        );
        return project?.name || '—';
    };
                    ...
                    <table className="w-full">
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {filteredMembers.map((member, index) => (
                                <tr key={member.id} className="hover:bg-gray-50 transition-colors animate-fade-in" style={{ animationDelay: `${index * 0.05}s` }}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="text-sm text-gray-600">{getProjectForMember(member.id)}</div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <button
