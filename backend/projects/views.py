...
#   GET    /projects/                    admin -> own projects only
#   GET    /projects/<id>/               admin (owner) OR manager/staff (member)
#   PATCH  /projects/<id>/               admin (owner) only
#   DELETE /projects/<id>/               admin (owner) only
#   GET    /projects/<id>/dashboard/     admin (owner) OR manager/staff (member)
...
#   Layer 1 -> Role permission class (IsAdminRole / IsAuthenticated)
#   Layer 2 -> Scope check inside view (_get_project_for_admin / _get_project_for_member)
# """
...
#     """
#     Fetch project and verify requesting user (any role) is a member.
#     Returns (project, error_response). error_response is None on success.
#     """
#     project = get_project_by_id(project_id)
#     if project is None:
...
# # GET  /projects/    - list   (admin: own projects | manager/staff: redirect info)
# # ─────────────────────────────────────────────────────────────────────────────
...
#         Manager/Staff -> return their single project info (they have no list).
#         """
#         if request.user.role == Role.ADMIN:
#             projects = get_projects_for_admin(request.user)
#             return Response(ProjectListSerializer(projects, many=True).data)

#         # Manager / Staff land here - return their project directly
...
#         Flow:
#           1. Validate input
#           2. Create project + auto-generate manager/staff
#           3. Fire Celery AI task (async)
#           4. Return 202 with project data + task_id
#         """
#         if request.user.role != Role.ADMIN:
#             return Response(
...
#             # Project is saved but AI can't run - inform the caller
#             logger.warning("No Gemini key for admin %s: %s", request.user.email, exc)
#             return Response(
#                 {
#                     "detail": str(exc),
...
#             logger.error("Failed to dispatch AI task for project %s: %s", project.pk, exc)
...
# # ─────────────────────────────────────────────────────────────────────────────
# # GET    /projects/<id>/   - detail (admin owner OR any project member)
# # PATCH  /projects/<id>/   - update (admin owner only)
# # DELETE /projects/<id>/   - delete (admin owner only, cascades members)
# # ─────────────────────────────────────────────────────────────────────────────
...
#         """
#         Any project member (admin, manager, staff) can read project details.
#         Scope enforced: user must belong to THIS project.
#         """
...
#         """Update project name/description. Admin owner only."""
#         if request.user.role != Role.ADMIN:
#             return Response(
...
#         # If description changed, re-trigger AI analysis
#         task_id = None
...
#         CASCADE: manager + staff users are deleted with the project.
#         """
#         if request.user.role != Role.ADMIN:
#             return Response(
...
# # Admin, manager, staff can all access - but only their own project's dashboard.
# # ─────────────────────────────────────────────────────────────────────────────
...
#         Returns project data + module flags so the frontend can render
#         the correct sidebar/modules.

#         Scope enforced: user must be a member of THIS project.
...
#         if project.ai_status == "pending" or project.ai_status == "processing":
#             return Response(
#                 {
#                     "detail": "Project setup is still in progress. Please wait for AI analysis to complete.",
...
#             return Response(
#                 {
#                     "detail": "AI analysis failed. Please contact your admin.",
#                     "ai_status": project.ai_status,
...
#         Returns the project's ai_status field + enabled modules.
#         Admin owner only - no need for members to poll this.
#         """
#         project, err = _get_project_for_admin(request, pk)
#         if err:
#             return err

#         # Optionally also poll Celery task if task_id was stored
#         task_id = request.query_params.get("task_id")
#         celery_state = None
#         if task_id:
#             celery_state = get_task_status(task_id)
...
#         }
#         if celery_state:
#             response_data["celery_state"] = celery_state

#         return Response(response_data)


# # ─────────────────────────────────────────────────────────────────────────────
# # PATCH /projects/<id>/manager/   - update manager credentials
# # PATCH /projects/<id>/staff/     - update staff credentials
# # Admin owner only.
# # ─────────────────────────────────────────────────────────────────────────────
